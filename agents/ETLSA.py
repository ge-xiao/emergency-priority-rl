...
class Node:
    ...
    def init_ETLSA(self):
        ...
        p1=plan[0][0]
        p2=plan[0][1]
        self.cur_duration=plan[0][2]
        self.cur_action=self.P[p1][p2]
    ...
    #得到最佳绿灯时间
    def getBestGreen(self,idx):
        lane=self.ilds_in[idx]
    ...
    def getPlan(self):
        scheduled = [False for i in range(8)]
        numDGreen = [0 for i in range(8)]
        plan = []
        lanes=self.ilds_in

        for i in range(8):
            numDGreen[i] = self.sim.lane.getLastStepVehicleNumber(lanes[i])
        ...
        for j in range(4):
            ...
            for i in range(8):
                if scheduled[i] == False:
                    if numDGreen[i] > maxValue:
                        maxValue = numDGreen[i]
                        maxIndex = i
            ...
            if len(self.EVs)>0:
                ...
                self.EVs.remove(vc)
                maxIndex=lanes.index(ev_lane)
                #print("安排",ev_lane)


            p = maxIndex
            p1 = self.unCon[maxIndex][0]
            p2 = self.unCon[maxIndex][1]

            if scheduled[p1] == True:
                # <p,p2>
                tup = (p, p2, max(self.getBestGreen(p), self.getBestGreen(p2)))
                plan.append(tup)
                scheduled[p] = True
                scheduled[p2] = True
                continue
            if scheduled[p2] == True:
                tup = (p, p1, max(self.getBestGreen(p), self.getBestGreen(p1)))
                plan.append(tup)
                scheduled[p] = True
                scheduled[p1] = True
                continue

            if numDGreen[p1] > numDGreen[p2]:
                # <p,p1>
                # <p2>
                tup = (p, p1, max(self.getBestGreen(p), self.getBestGreen(p1)))
                plan.append(tup)
    ...
    def updatePhase(self):
        ...
        if self.since_last>=self.cur_duration:
            ...
            p1 = self.plan[self.cur_stage][0]
            p2 = self.plan[self.cur_stage][1]
            self.cur_duration = self.plan[self.cur_stage][2]
            self.cur_action = self.P[p1][p2]
...
if __name__ == '__main__':
    ...
    config.read('../config/test_5x5/ETLSA.ini')
    #config.read('../config/test_5x5_2/ETLSA.ini')

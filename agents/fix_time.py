...
class Evaluate():
    ...
    def test(self):
        ...
        for seed in self.evaluate_seed:
            ...
            self.env.run()
            if self.num_ev > 0:
                self.env.saveResult("fix_time_"+str(self.num_ev), seed)
            else:
                self.env.saveResult("fix_time", seed)


if __name__ == '__main__':
    config = configparser.ConfigParser()
    config.read('../config/test_5x5/fix_time.ini')
    #config.read('../config/test_5x5_2/fix_time.ini')
    #config.read('../config/hangzhou/fix_time.ini')
    #config.read('../config/manhattan/fix_time.ini')

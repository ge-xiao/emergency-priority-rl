...
import json
import os
from environment.EMVLight_node import Node
...
class TrafficSimulator:
    ...
    def get_state(self):
        ...
        for node_name in self.sorted_nodes:
            node = self.nodes[node_name]
            inlanes,outlanes,d_ev=node.get_node_state()


            cur_state = inlanes + outlanes+d_ev
            cur_state_mp[node_name]=cur_state
            node.n_s = len(cur_state)


        #遍历，得到node 是否为second,更新参数
        for node_name in self.sorted_nodes:
            node = self.nodes[node_name]
            if node.is_p==True:
                t_node=node.next
                if t_node in self.sorted_nodes:
                    self.nodes[t_node].is_s=True
                    self.nodes[t_node].from_n=node_name
        ...
        for node_name in self.sorted_nodes:
            ...
            for nnode_name in node.neighbor:
                ...
                cur_state=cur_state+cur_state_mp[nnode_name]+nnode.fingerprint

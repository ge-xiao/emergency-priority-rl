...
class Dynamic_route():
    ...
    def run(self):
        ...
        while step<3600:
            ...
            if step%1==0:
                for lane in self.tot_ilds_in['t_1_5']:
                    #print(traci.lanearea.getLastStepVehicleIDs(lane))
                    if lane=="p_0_5-t_1_5_0":
                        print(lane)
                        print(self.sim.inductionloop.getLastStepVehicleIDs(lane))
            ...
            if step == 50:
                ...
                start_v=self.name_idx['t_1_5']
                to_v=self.name_idx['t_5_1']
                D, previousVertex = g.dijkstra(start_v)

                # 每个节点的前节点，可通过回溯得到最短路径
                for vertex in range(len(D)):
                    print(f"distance from vertex to vertex {vertex} is {D[vertex]}")
                ...
                real_path=[]
                for point in path[:: -1]:
                    real_path.append(self.sorted_nodes[point])
                print(real_path)
